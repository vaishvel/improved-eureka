...
#To obtain Weakness Classes under View 1000 from View
for views in CWE_root[2][0]:
    for members in views:                                                
        if members.tag=="{http://cwe.mitre.org/cwe-6}Has_Member":          
            cwe_id = members.get('CWE_ID')                              
            child_of = '1000'
            for weakness in CWE_root[0]:
...
for weakness in CWE_root[0]:
     if weakness.attrib['Status']!="Deprecated":
        for parameter in weakness:
            if parameter.tag=="{http://cwe.mitre.org/cwe-6}Related_Weaknesses":
                for related_weakness_parameter in parameter:
                    if related_weakness_parameter.attrib['CWE_ID']==cwe_id:     #related_weakness_parameter.find('ChildOf')
                        cwe_id= weakness.attrib['ID']
                        cwe_name= remove_commas(weakness.attrib['Name'])
                        child_of= related_weakness_parameter.attrib['CWE_ID']
                        temp_list = list()
                        temp_list.extend((cwe_id,cwe_name,child_of))
                        CWE_entries.append(temp_list)
